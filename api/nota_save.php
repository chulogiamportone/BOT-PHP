...
// Si tu config.php ya crea $pdo, úsalo. Si no, construimos PDO con $dsn/$user/$pass.
if (!isset($pdo)) {
...
$imagenFinal = $imagenUrl; // Por defecto usar la URL si la ingresaron
...
if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
    ...
    $carpetaDestino = __DIR__ . '/../uploads/notas/';
    if (!is_dir($carpetaDestino)) {
        mkdir($carpetaDestino, 0755, true);
    }
    
    // Generar nombre único
    $nombreArchivo = uniqid('nota_', true) . '.' . $extension;
    $rutaCompleta = $carpetaDestino . $nombreArchivo;
    
    // Mover archivo
    if (move_uploaded_file($archivo['tmp_name'], $rutaCompleta)) {
        // Guardar la URL relativa
        $imagenFinal = '/uploads/notas/' . $nombreArchivo;
    } else {
        http_response_code(500);
...
// subsecciones llegan como arreglos paralelos subsecciones[titulo][], subsecciones[contenido][]
...
try {
    // Asegurar que la tabla exista (opcionalmente podés quitar esto en prod)
    $pdo->exec("
    ...
    // Usamos INSERT ... ON DUPLICATE KEY UPDATE en lugar de REPLACE para no borrar la fila
    $sql = "
        INSERT INTO notas (id, titulo, imagen, texto, subsecciones)
        VALUES (?, ?, ?, ?, ?)
    ...
    http_response_code(500);
    echo json_encode(['error' => 'Error al guardar nota: ' . $e->getMessage()]);
}